...
public class cchip
{
	...
	public static ReadHandlerPtr cchip1_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		int ret = 0;
		...
		switch (offset)
		{
			...
			case 0x004:
				/* Coins */
				if (errorlog != null) fprintf (errorlog, "cchip1_r (coin) pc: %06x, offset: %04x\n", cpu_get_pc(), offset);
				if (cchip1_bank == 1)
					ret = cchip1_code[offset/2];
				else
					if (cchip[2]!=0)
			...
			default:
				...
				{
					if (errorlog != null) fprintf (errorlog, "cchip1_r offset: %04x\n", offset);
					ret = 0xff;
				}
				break;
		}
	...
	public static WriteHandlerPtr cchip1_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (errorlog != null) fprintf (errorlog, "cchip1_w pc: %06x, %04x:%02x\n", cpu_get_pc(), offset, data);
		switch (offset)
		{
			case 0x0000:
				if ((data & 0xff) == 0x4a)
