...
public class berzerk
{
	...
	public static final int SOUND_CHANNEL =1;    /* channel for sound effects */
	public static final int VOICE_CHANNEL =5;    /* special channel for voice sounds only */
	public static final int DEATH_CHANNEL =6;    /* special channel for fake death sound only */
	...
	if (errorlog != null) fprintf(errorlog,"Trying death sound");
					switch(deathsound)
					{
					case 1 :
						sample_start(2,33,0);
			...
			if (offset==4)               /* Game voice sounds */
			{
				if (deathsound<2)	       /* no voices for real death sound */
				{
					...
					sample_set_freq(VOICE_CHANNEL,(int)samplefrequency);
	...
	{
		if (errorlog != null)  fprintf(errorlog, "B Data value %d and offset %d at %d\n", data, offset, lastfreq);
	} };
	
	public static ShUpdatePtr berzerk_sh_update = new ShUpdatePtr() { public void handler() 
	{
