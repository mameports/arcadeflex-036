...
public class psychic5
{
	...
	public static WriteHandlerPtr set_visible_colors = new WriteHandlerPtr() { public void handler(int bg_scrollx, int bg_scrolly)
	{
		...
		for (x = 0; x < 16; x++)
		{
			...
			if (temp != 0)
				for (y = 0; y < 16; y++)
					if ((temp & (1 << y))!=0) palette_used_colors.write(256 + 16 * x + y,PALETTE_COLOR_USED);
		}
		...
		for (x = 0; x < 16; x++)
		{
			...
			{
				for (y = 0; y < 15; y++)
					if ((temp & (1 << y))!=0) palette_used_colors.write(512 + 16 * x + y,PALETTE_COLOR_USED);
		...
		for (x = 0; x < 16; x++)
		{
			...
			{
				for (y = 0; y < 15; y++)
					if ((temp & (1 << y))!=0) palette_used_colors.write(0 + 16 * x + y,PALETTE_COLOR_USED);
