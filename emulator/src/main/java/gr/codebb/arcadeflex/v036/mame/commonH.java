...
public class commonH {
    ...
    /* ROM to load */
    public static void ROM_LOAD(String name,int offset,int size,int crc){arload.add(new RomModule( name,offset,size,crc));}
    /* continue loading the previous ROM to a new address */
    public static void ROM_CONTINUE(int offset,int length){arload.add(new RomModule( null,offset,length,0));}
...
/*TODO*/ //#define ROM_RELOAD_NIB_HIGH(offset,length) { (char *)-1, offset, (length) | ROMFLAG_NIBBLE | ROMFLAG_ALTERNATE, 0 },
    ...
    public static void ROM_RELOAD_EVEN(int offset,int length){arload.add(new RomModule( "-1",(offset) & ~1, (length) | ROMFLAG_ALTERNATE,0));}
