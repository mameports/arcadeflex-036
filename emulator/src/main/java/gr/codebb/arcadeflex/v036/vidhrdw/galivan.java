...
public class galivan
{
	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr galivan_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	{
		...
		{
			if ((i & 8) != 0) colortable[Machine.drv.gfxdecodeinfo[1].color_codes_start + i] = (char) (192 + (i & 0x0f) + ((i & 0xc0) >> 2));
			else colortable[Machine.drv.gfxdecodeinfo[0].color_codes_start + i] = (char) (192 + (i & 0x0f) + ((i & 0x30) >> 0));
		}
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr get_char_tile_info = new WriteHandlerPtr() { public void handler(int col, int row)
	{
		...
		tile_info.priority = (attr & 8)!=0 ? (char)0 : (char)1;	/* wrong */
	} };
	
	public static WriteHandlerPtr ninjemak_get_bg_tile_info = new WriteHandlerPtr() { public void handler(int col, int row)
	{
		UBytePtr BGROM = memory_region(REGION_GFX4);
		int addr = 32 * col + row;
		int attr = BGROM.read(addr + 0x4000);
	...
	public static WriteHandlerPtr ninjemak_get_char_tile_info = new WriteHandlerPtr() { public void handler(int col, int row)
	{
		int addr = 32 * col + row;
		int attr = colorram.read(addr);
	...
	***************************************************************************/
	...
	if (errorlog != null) fprintf(errorlog,"%04x: write %02x to port 80\n",cpu_get_pc(),data);
	...
	***************************************************************************/
	...
	public static VhUpdatePtr galivan_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	...
	public static VhUpdatePtr ninjemak_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
