...
public class toaplan1 {
    ...
    static int toaplan1_coin_count;
    /* coin count increments on startup ? , so dont count it */
    ...
    public static ReadHandlerPtr demonwld_dsp_in = new ReadHandlerPtr() {
 ...
 /*unsigned*/ int input_data = 0;
            ...
            switch (main_ram_seg) {
                case 0xc00000:
                    input_data = cpu_bankbase[1].READ_WORD(dsp_addr_w);//input_data = READ_WORD( & (cpu_bankbase[1][(dsp_addr_w)]));
                    break;
            ...
            if (errorlog != null) {
                fprintf(errorlog, "DSP PC:%04x IO read %04x at %08x (port 1)\n", cpu_getpreviouspc(), input_data, main_ram_seg + dsp_addr_w);
            }
            return input_data;
        }
    };

    public static WriteHandlerPtr demonwld_dsp_out = new WriteHandlerPtr() {
        public void handler(int fnction, int data) {
            if (fnction == 0) {
 ...
 /*		to move it to an even address word boundary */
                ...
                if (errorlog != null) {
                    fprintf(errorlog, "DSP PC:%04x IO write %04x (%08x) at port 0\n", cpu_getpreviouspc(), data, main_ram_seg + dsp_addr_w);
                }
            }
            if (fnction == 1) {
                ...
                switch (main_ram_seg) {
                    case 0xc00000:
                        cpu_bankbase[1].WRITE_WORD(dsp_addr_w, data);
                ...
                if (errorlog != null) {
                    fprintf(errorlog, "DSP PC:%04x IO write %04x at %08x (port 1)\n", cpu_getpreviouspc(), data, main_ram_seg + dsp_addr_w);
                }
            }
            if (fnction == 3) {
 ...
 /*				communication to main processor*/
                if (errorlog != null) {
                    fprintf(errorlog, "DSP PC:%04x IO write %04x at port 3\n", cpu_getpreviouspc(), data);
                }
                if ((data & 0x8000) != 0) {
                ...
                if (data == 0) {
                    if (dsp_execute != 0) {
                        if (errorlog != null) {
                            fprintf(errorlog, "Turning 68000 on\n");
                        }
    ...
    public static WriteHandlerPtr demonwld_dsp_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            /*#if 0
		if (errorlog != 0) fprintf(errorlog,"68000:%08x  Writing %08x to %08x.\n",cpu_get_pc() ,data ,0xe0000a + offset);
	#endif*/
            ...
            switch (data) {
                case 0x0000:
                    ...
                    if (errorlog != null) {
                        fprintf(errorlog, "Turning DSP on and 68000 off\n");
                    }
                ...
                case 0x0001:
                    ...
                    if (errorlog != null) {
                        fprintf(errorlog, "Turning DSP off\n");
                    }
    ...
    public static WriteHandlerPtr toaplan1_int_enable_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            toaplan1_int_enable = data;
        }
    };

    public static ReadHandlerPtr toaplan1_unk_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return unk ^= 1;
        }
    };

    public static ReadHandlerPtr samesame_port_6_r = new ReadHandlerPtr() {
        public int handler(int offset) {
    ...
    public static ReadHandlerPtr vimana_input_port_5_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int data, p;
 ...
 /* credit per keypress */
    ...
    public static ReadHandlerPtr vimana_mcu_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int data = 0;
            switch (offset >> 1) {
                case 0:
    ...
    public static WriteHandlerPtr vimana_mcu_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            switch (offset >> 1) {
                case 0:
                    break;
    ...
    public static ReadHandlerPtr toaplan1_shared_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return toaplan1_sharedram.read(offset >> 1);
        }
    };

    public static WriteHandlerPtr toaplan1_shared_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr rallybik_coin_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            switch (data) {
                case 0x08:
                    if (toaplan1_coin_count != 0) {
    ...
    public static WriteHandlerPtr toaplan1_coin_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if (errorlog != null) {
                fprintf(errorlog, "Z80 writing %02x to coin control\n", data);
            }
 ...
 /* The following are coin counts after coin-lock active (faulty coin-lock ?) */
                ...
                case 0xe8:
                    break;
                /* ??? Maximum credits reached with coin/credit ratio */
                case 0xe4:
                    break;
                /* ??? Reset coin system */
