...
public class tnzs
{
	...
	public static ReadHandlerPtr arkanoi2_sh_f000_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		int val;
	
		if (errorlog != null) fprintf (errorlog, "PC %04x: read input %04x\n", cpu_get_pc(), 0xf000 + offset);
	...
	static void mcu_handle_coins(int coin)
	{
		...
		else if (coin!=0 && coin != insertcoin)
		{
			if ((coin & 0x01) != 0)	/* coin A */
			{
				...
				{
					coin_lockout_global_w.handler(0,1); /* Lock all coin slots */
				}
				else
				{
					if (errorlog != null) fprintf (errorlog, "Coin dropped into slot A\n");
					coin_lockout_global_w.handler(0,0); /* Unlock all coin slots */
			...
			if ((coin & 0x02) != 0)	/* coin B */
			{
				...
				{
					if (errorlog != null) fprintf (errorlog, "Coin dropped into slot B\n");
					coin_lockout_global_w.handler(0,0); /* Unlock all coin slots */
			...
			{
				if (errorlog != null) fprintf (errorlog, "Coin dropped into service slot C\n");
				mcu_credits++;
			}
			mcu_reportcoin = coin;
		}
	...
	public static ReadHandlerPtr mcu_arkanoi2_r = new ReadHandlerPtr() { public int handler(int offset)
	{
	...
	//	if (errorlog != 0) fprintf (errorlog, "PC %04x: read mcu %04x\n", cpu_get_pc(), 0xc000 + offset);
		...
		if (offset == 0)
		{
			...
			switch (mcu_command)
			{
				...
				default:
                                    if (errorlog != null) fprintf (errorlog, "error, unknown mcu command\n");
					/* should not happen */
					return 0xff;
					//break;
			}
	...
	public static WriteHandlerPtr mcu_arkanoi2_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (offset == 0)
		{
...
/*TODO*///		if (errorlog != null) fprintf (errorlog, "PC %04x (re %04x): read mcu %04x\n", cpu_get_pc(), cpu_geturnpc(), 0xc000 + offset);
		...
		if (offset == 0)
		{
			...
			switch (mcu_command)
			{
				...
				default:
                                    if (errorlog != null) fprintf (errorlog, "error, unknown mcu command (%02x)\n",mcu_command);
					/* should not happen */
					return 0xff;
					//break;
			}
...
/*TODO*///			if (errorlog != null) fprintf (errorlog, "PC %04x (re %04x): write %02x to mcu %04x\n", cpu_get_pc(), cpu_geturnpc(), data, 0xc000 + offset);
			...
			if ((data == 0x09) && (mcu_type == MCU_DRTOPPEL))
				mcu_credits = (char)((mcu_credits - 1) & 0xff);		/* Player 1 start */
			if ((data == 0x18) && (mcu_type == MCU_DRTOPPEL))
				mcu_credits = (char)((mcu_credits - 2) & 0xff);		/* Player 2 start */
